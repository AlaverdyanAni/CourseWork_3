...
SELECT  students.name, students.age,faculties.name FROM students INNER JOIN faculties ON students.faculty_id = faculties.id;
SELECT students.name, students.age From students LEFT Join avatar on students.id = avatar.student_id;